    ...
    public static class Tools
    {
        ...
        public static void SendMail(string to, string subject, string body)
        {
            ...
            })
            {
                ...
                using (SmtpClient smtp = new SmtpClient("smtp.office365.com", 587) {EnableSsl = true, UseDefaultCredentials = true, DeliveryMethod = SmtpDeliveryMethod.Network, Credentials = new NetworkCredential(config["email"]["username"].ToString(), config["email"]["password"].ToString())})
                {
                    try
                    {
                        smtp.Send(mail);
